...
class Database:
    ...
    def get_all_orders(self):
        ...
        try:
            ...
            cursor.execute("""
                SELECT o.id, o.user_id, o.product_key, o.color, o.quantity, 
                       o.total_price, o.status, o.payment_status, o.tracking_link, 
                       o.created_at, p.name, u.username, u.first_name, o.delivery_address
                FROM orders o
                JOIN products p ON o.product_key = p.product_key
    ...
    def update_order_status(self, order_id, status, payment_status=None, tracking_link=None):
        """Update order status"""
        conn = self.get_connection()
        try:
            cursor = conn.cursor()
            
            if payment_status and tracking_link:
                cursor.execute("""
                    UPDATE orders 
                    SET status = ?, payment_status = ?, tracking_link = ?
                    WHERE id = ?
                """, (status, payment_status, tracking_link, order_id))
            elif payment_status:
                cursor.execute("""
                    UPDATE orders 
